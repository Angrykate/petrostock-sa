...
export default function Layout({ user, currentPage, onNavigate, onLogout, children, alertCount, notifications, notificationsOpen, onToggleNotifications, onDismissNotification }: LayoutProps) {
        ...
        <header className="flex items-center justify-between px-5 py-3 border-b shrink-0"
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <button ref={bellRef} onClick={onToggleNotifications} className="relative w-9 h-9 flex items-center justify-center rounded-lg border transition-colors hover:bg-white/5"
              ...
              {notificationsOpen && (
                <div ref={notifRef} className="fixed right-0 mt-2 w-80 rounded-2xl border bg-[#090d15] shadow-2xl" style={{ borderColor: '#1c2540', zIndex: 9999, top: '60px' }}>
                  ...
                  <div className="max-h-72 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      <div key={note.id} className="px-4 py-3 border-b last:border-0 cursor-pointer hover:bg-white/5" style={{ borderColor: '#1c2540' }} onClick={() => { onDismissNotification?.(note.id); onToggleNotifications() }}>
